...
#NYC$INJUREDORNOT[NYC$KILLED>0]=1
#NYC$INJUREDORNOT[NYC$INJURED==0]=0
...
fit<-randomForest(factor(INJUREDORNOT)~.,training,importance=TRUE, ntree=100)
importance(fit)
varImpPlot(fit)
Prediction<- predict(fit,test)
table(actual=test$INJUREDORNOT,Prediction)
wrong<- (test$INJUREDORNOT!=Prediction)
