...
library('C50')
mytree <- C5.0( factor(INJUREDORNOT)~., data =training )

summary(mytree)
#dev.off()
plot(mytree)
prediction<-predict( mytree ,test , type="class" )
pereva<-table(actual=test[,22],prediction)# create the confusion matrix
pereva
####error rate####
...
rate
#####correctly predicted######
diag<-diag(pereva)# number of correctly classified instances per class
diag
####accuracy###
n<-sum(pereva)# number of instances
acc<-sum(diag)/n
acc
######
rowsums <- apply(pereva, 1, sum) # number of instances per class
colsums <- apply(pereva, 2, sum) # number of predictions per class
...
##########
#plot(NYC$TIME,NYC$INJURED,col="blue", xlab ="TIME", ylab ="INJURED", main = "Line_Plot", type="b")
?plot()
#lines(NYC$TIME,NYC$INJURED,col="blue", xlab ="TIME", ylab ="INJUREDORNOT", main = "Line_Plot", type="b")
#########
#plot(NYC[,c(19,7,25,4,8,9)],col="darkgreen",las=1,main="Scatte_Plot")

source("http://bioconductor.org/biocLite.R")
biocLite("hexbin")
library(hexbin)
a<-hexbin(NYC$KILLED,NYC$INJURED,xbins=40)
library(RColorBrewer)
plot(a)
######
mosaicplot(NYC$BOROUGH,NYC$INJUREDORNOT)
